...
do 
    if [ "$header" = false ]; then 
        ...
        ##Need to work on effective pom.xml
        ...
        semicolon_found_indicates_file=$(echo  $unused_dir | grep ";" | wc -l)
        if [[ $semicolon_found_indicates_file -eq 0 ]]; then
            ...
            do
                ...
                echo $JAVA_HOME
                echo ${mvn_command} 
                ${mvn_command} > "$currentDir/$logs/log_${last_level_dir}_${plugin_start}.txt"
